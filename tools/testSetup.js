 ...
 */
process.env.NODE_ENV = 'test';   //we are setting node environment variable to test so that 
//develop-specific features like hot reloading are disabled when we are running tests.
...
// Configure JSDOM(which will provide a virtual in-memory DOM for us. This will allow us to test react components without having to open the browser) 
//and set global variables(like window, document, navigator) to simulate a browser environment for tests.React looks for these to determine if it's in the browser, so it's imp to have these available when doing DOM-based testing in React.
